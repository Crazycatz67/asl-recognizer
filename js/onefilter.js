// js/onefilter.js — the One Euro Filter (Casiez, Roussel & Vogel, 2012;
...
//
// The tracker's raw hand landmarks jitter frame to frame even when the hand
// is dead still (sensor/model noise), so main.js has always smoothed them —
// previously with a fixed-alpha EMA (constant blend weight regardless of
// speed). A fixed alpha has to compromise: heavy enough to kill still-hand
// jitter, but that same heaviness adds visible lag once the hand actually
// moves ("ghosting" — the drawn skeleton visibly trailing the real hand).
// One-euro instead widens its own cutoff frequency (== responds faster,
// smooths less) in proportion to the signal's estimated velocity: nearly
// motionless -> heavy smoothing; moving fast -> mostly raw, low lag. Same
// two knobs either way, just chosen per-frame instead of once.
//
...
// (signal units per second) into an added cutoff — see the module comment
// on why these three specific starting values are an informed guess, not a
// measured one. Handles a variable-length point array (built lazily on the
// first real sample) and a `null` input (hand lost) by forgetting state, the
// same contract main.js's old smoothLandmarks() had.
