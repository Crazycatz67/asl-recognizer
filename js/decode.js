...
export function createDecoder(lexicon, opts = {}) {
  ...
  function advance(beam, o, c) {
    ...
    // De-dup key: (committed-word count, current partial word, is-a-word).
    // `cur` fixes the trie node, and since the language model is unigram,
    // two hypotheses that differ only in earlier words can never be
    // re-ranked later — the lower one is dropped. (The per-word insertion
    // budget `ins` is NOT in the key, so a used-up hypothesis can shadow an
    // equal-text one that still has its insertion — a small approximation.)
    const put = (cand) => {
