...
| 5 | **Challenge** | Hide "seeing X" during play. Combo multiplier ×1→×4. Normal/Hard difficulty (confusables, 3–5-letter words later). A fair wrong-letter time drain. End screen with slowest letters and "Practice these". Persisted bests. Fix the start-card copy and use `START_LIVES`. | **DONE** `0abc510`, pushed — needs live confirm. No peeking, combo ×1–×4, Normal/Hard, confusables + words ramp, fair 2× drain + near grace, end summary w/ practice links, per-difficulty bests. |
| 6 | **Sound design** | Distinct synthesized families (close-tick, letter-lock, word cue, combo ladder, countdown, life lost, game over, NEW BEST, run complete). An optional music bed in Challenge. A visual twin for every sound. | **DONE** `0abc510` (+ `e582e17` word cue/cooldowns). Distinct cues per event, each with a visual twin. Music bed NOT done (optional). |
| 7 | **Onboarding & understanding** | **7a:** interactive ~75 s first-run tour (`js/tour.js`). **7b:** `palmFacing()` orientation cue, badge and hints, plus rewritten `LETTER_GUIDE` copy. **7c:** colorblind-safe overlay with redundant encoding and a color-key chip. **7d:** mirror/viewer toggle, auto-flip hysteresis, "show me slowly". **7e:** copy rewrite. **7f:** confidence ring, tricky-letters review, mastery map. | **7a/7c/7d/7e DONE** (`ffb4f6c` `628c57f` `4444fc8` `81c6d94`), pushed — needs live confirm. 7b palm-facing detection, Mirror/Viewer toggle, "Show me slowly", 7f NOT DONE. |
| 8 | **Presentation framing** | `about.html` + intro framed as a **fingerspelling practice tool** (not "ASL translation"), complementary to Google's Sign-to-Text. Credit PopSign. Invite Deaf testers. Bump `sw.js` VERSION on every deploy. | **DONE** `ff81d0e` + `4dd0689` (About page: Google Sign-to-Text context, facts checked against sources; PopSign credit fixed). |
